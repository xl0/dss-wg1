function M = react(M_old, P)
	...
	while (n < M.x_num)
		...
		M.aPAR(n) = M.aPAR(n) + P.delta_t * P.Kon_A * M.Ro_cyto_aPAR;% * (P.delta_x / P.L); % Cyto ->  Membrane
...
%		M.pPAR(n) = M.pPAR(n) + P.delta_x * P.delta_t * P.Kon_P * M.Ro_cyto_pPAR / P.L;
	...
	M.Ro_cyto_aPAR = M.Ro_cyto_aPAR - P.delta_t * P.Kon_A * M.Ro_cyto_aPAR + ... % Cyto -> membrane term.
		P.delta_t * P.delta_x * P.Koff_A;% * (sum(M_old.aPAR) / P.L);
